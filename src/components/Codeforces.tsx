...
initialData.datasets[0].backgroundColor = initialData.datasets[0].data.map((val:number) => settings.color.light(val))
initialData.datasets[0].pointHoverBackgroundColor = initialData.datasets[0].data.map((val:number) => settings.color.light(val))
...
export default () => {
    ...
    useEffect( () => {
        const effect = async () => {
            ...
            const bgColor = rating.map((val:any) => { return settings.color[colorMode](val) })
